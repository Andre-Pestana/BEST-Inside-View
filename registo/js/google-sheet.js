...
var request;
	...
	function start(){
		...
		document.getElementById("text").innerHTML='(0/2) Submetendo....';
...
$("#foo").submit(function(event){
	...
	/*
	if(status==0){
		alertify.error("Não te esqueças de submeter o currículo!");
	...
	if(first.getAttribute("name")==""){
		alertify.error("Tens de escolher pelo menos uma empresa!");
		event.preventDefault();
		end();
		return;
	}
	...
	if(document.getElementById('Reg').checked==false){
		alertify.error("Tens de aceitar o nosso regulamento!");
		event.preventDefault();
		end();
		return;
	}
    ...
    request.done(function (response, textStatus, jqXHR){
        ...
        console.log(jqXHR);
		alertify.success('Inscrição submetida com sucesso!');
		end();
    });
